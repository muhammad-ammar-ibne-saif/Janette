...
export const BOOK_URL ="https://janetteleechiropractor.janeapp.co.uk/#staff_member/1";
  ...
  return (
    ...
    >
      <div className="container-page flex items-center justify-between py-3">
        <Link to="/" className="flex items-center gap-3 group" aria-label="Janette Lee Chiropractor Peterborough — Home">
          <Logo className="h-12 w-auto sm:h-14 md:h-24" />
        </Link>
          ...
          <nav className="container-page mt-6 flex flex-col gap-1 pb-10">
            {nav.map((n, i) => (
              <Link
                ...
                hash={"hash" in n ? n.hash : undefined}
                onClick={() => setOpen(false)}
                className="border-b border-border/60 py-4 font-display text-2xl hover:text-primary transition-colors"
              >
                {n.label}
              </Link>
            ))}
    ...
    <footer className="border-t border-border/70 bg-card">
      <div className="container-page py-14">
        <div className="grid gap-10 md:grid-cols-4">
          <div className="md:col-span-1">
            ...
            <p className="mt-4 text-sm text-muted-foreground">
              GCC Registered Chiropractor in Peterborough offering personal, gentle care for all ages — nearly four decades of experience.
            </p>
          </div>

          <div>
            <div className="text-xs uppercase tracking-wider text-gold">Navigate</div>
            <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
              <li><Link to="/" className="hover:text-foreground">Home</Link></li>
              <li><Link to="/about" className="hover:text-foreground">About Janette</Link></li>
              <li><Link to="/services" className="hover:text-foreground">Services</Link></li>
              <li><Link to="/services" hash="conditions" className="hover:text-foreground">Conditions</Link></li>
              <li><Link to="/contact" className="hover:text-foreground">Contact</Link></li>
            </ul>
          </div>

          <div>
            <div className="text-xs uppercase tracking-wider text-gold">Book</div>
            <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
              <li><a href={BOOK_URL} target="_blank" rel="noreferrer" className="hover:text-foreground">Book Online (Jane App)</a></li>
              <li><a href={`tel:${PHONE_TEL}`} className="hover:text-foreground">Call to book</a></li>
              <li><a href={`mailto:${EMAIL}`} className="hover:text-foreground">Email to book</a></li>
            </ul>
          </div>

          <div>
            <div className="text-xs uppercase tracking-wider text-gold">Contact</div>
            <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
              <li className="flex items-start gap-2"><Phone className="h-4 w-4 mt-0.5 shrink-0 text-gold" /><a href={`tel:${PHONE_TEL}`} className="hover:text-foreground">{PHONE_DISPLAY}</a></li>
              <li className="flex items-start gap-2"><Mail className="h-4 w-4 mt-0.5 shrink-0 text-gold" /><a href={`mailto:${EMAIL}`} className="hover:text-foreground break-all">{EMAIL}</a></li>
              <li className="flex items-start gap-2"><MapPin className="h-4 w-4 mt-0.5 shrink-0 text-gold" /><span>{ADDRESS_FULL}</span></li>
            </ul>
          </div>
        </div>
        ...
        <div className="mt-12 flex flex-col gap-2 border-t border-border/70 pt-6 text-xs text-muted-foreground sm:flex-row sm:items-center sm:justify-between">
          <div>© 2026 Janette Lee Chiropractor Peterborough · All rights reserved</div>
...
export function CTASection() {
  return (
    <section className="py-16 md:py-20" style={{ background: "var(--gradient-sage)" }}>
      <div className="container-page text-center">
        <h2 className="font-display text-3xl sm:text-4xl md:text-5xl text-foreground">Ready to feel better?</h2>
        <p className="mt-4 max-w-xl mx-auto text-foreground/70">
          Book online with Jane App, call directly, or send a quick enquiry — I respond within one working day.
        </p>
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          ...
          <Link to="/contact" className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-7 py-3.5 text-sm font-medium text-foreground hover:-translate-y-0.5 transition-all">
